public abstract class Mover
    ...
    public Point nextPosition(int world, Point destination){
        int horizontal = Mover.sign(destination.x - this.getPosition());
        Point newPt = new Point(this.getPosition().getX() + horizontal, this.getPosition.getY());
        
        if (horizontal == 0 or !this.canMove(4, newPt)){
        	...
        	if (vertical == 0 or !this.canMove(4, newPt)){
    ...
    public Point toTarget(int world, Positionable destination){
    ...
    private static int adjacent(Point one, Point two){
